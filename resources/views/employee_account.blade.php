...
@section('title', 'تقرير حساب الموظف')
...
@section('content')
    <div class="max-w-6xl mx-auto bg-white p-6 mt-10 rounded-lg shadow-md">
        <h2 class="text-xl font-semibold mb-6 text-gray-700">تقرير حساب الموظف</h2>

        <form action="{{ route('reports.employee_account.result') }}" method="POST" class="grid grid-cols-1 md:grid-cols-3 gap-6">
            @csrf
            ...
            <div>
                <label class="block text-sm font-medium text-gray-600 mb-1">الموظف</label>
                <select name="employee_id" id="employee_id" class="w-full select2-employee" required>
            ...
            <div>
                <label class="block text-sm font-medium text-gray-600 mb-1">تاريخ البداية (التوريدات)</label>
                <input type="date" name="start_date" class="w-full p-2 border rounded-lg">
            </div>

            <div>
                <label class="block text-sm font-medium text-gray-600 mb-1">تاريخ النهاية (التوريدات)</label>
