...
<body class="bg-gray-100 p-6">
    ...
    <div class="max-w-6xl mx-auto bg-white p-6 mt-10 rounded-lg shadow-md">
        <h2 class="text-xl font-semibold mb-6 text-gray-700">تقرير الحسابات</h2>

        <form action="{{ route('reports.debt.result') }}" method="POST" class="grid grid-cols-1 md:grid-cols-3 gap-6">
            @csrf
            <!-- اختيار العميل -->
            <div>
                <label class="block text-sm font-medium text-gray-600 mb-1">العميل</label>
                <select name="client_id" id="client_id" class="w-full select2">
