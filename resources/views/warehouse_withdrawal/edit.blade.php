...
@section('content')
    <div class="max-w-3xl mx-auto bg-white rounded-2xl shadow-lg p-6">
        <div class="flex justify-between items-center mb-6">
            <h2 class="text-2xl font-bold text-gray-800">تعديل السحب</h2>
        ...
        <form id="withdrawalForm" action="{{ route('warehouse_withdrawals.update', $warehouse_withdrawal) }}" method="POST" class="space-y-4">
            @csrf
            @method('PUT')
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <select name="warehouse_id" id="warehouse_id" class="w-full select2" required>
                        ...
                        @foreach($warehouses as $wh)
                            <option value="{{ $wh->id }}" {{ old('warehouse_id', $warehouse_withdrawal->warehouse_id) == $wh->id ? 'selected' : '' }}>{{ $wh->name }}</option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select name="fuel_type" id="fuel_type" class="w-full p-2 border rounded-lg" required>
                        ...
                        <option value="1" {{ old('fuel_type', $warehouse_withdrawal->fuel_type) == 1 ? 'selected' : '' }}>جازولين</option>
                        <option value="2" {{ old('fuel_type', $warehouse_withdrawal->fuel_type) == 2 ? 'selected' : '' }}>بنزين</option>
                    </select>
                ...
                <div>
                    ...
                    <input type="text" name="driver_name" value="{{ old('driver_name', $warehouse_withdrawal->driver_name) }}" required
                ...
                <div>
                    ...
                    <input type="text" name="car_number" value="{{ old('car_number', $warehouse_withdrawal->car_number) }}" required
                        class="w-full p-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary">
                ...
                <div>
                    ...
                    <input type="text" name="amount" value="{{ old('amount', $warehouse_withdrawal->amount) }}" required
                        class="w-full p-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary">
                    @error('amount') <p class="text-red-500 text-sm mt-1">{{ $message }}</p> @enderror
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">التاريخ</label>
                    <input type="date" name="date" value="{{ old('date', $warehouse_withdrawal->date->toDateString()) }}" required
                        class="w-full p-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary">
            ...
            <div>
                ...
                <textarea name="note" rows="2" class="w-full p-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary">{{ old('note', $warehouse_withdrawal->note) }}</textarea>
            </div>
...
@section('scripts')
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            var currentStock = 0;
            var oldAmount = {{ $warehouse_withdrawal->amount }};
            ...
            function fetchStock() {
                ...
                if (warehouseId && fuelType) {
                    $.get('{{ route("api.warehouse-stock") }}', { warehouse_id: warehouseId, fuel_type: fuelType }, function(data) {
                        ...
                        $('#stockValue').text(fmt(currentStock + oldAmount));
            ...
            $('#withdrawalForm').on('submit', function(e) {
                ...
                var available = currentStock + oldAmount;
                ...
                if (amount > available) {
                    Swal.fire({
                        ...
                        html: 'الكمية المطلوبة: <b>' + fmt(amount) + '</b><br>الرصيد المتوفر: <b>' + fmt(available) + '</b>',
                        confirmButtonText: 'حسناً'
                    });
                    return;
                }
                ...
                Swal.fire({
                    icon: 'question',
                    title: 'تأكيد التعديل',
                    html: 'هل تريد تعديل السحب إلى <b>' + fmt(amount) + '</b> من ' + fuelTypeText + '<br>الرصيد المتوفر: <b>' + fmt(available) + '</b><br>الرصيد بعد التعديل: <b>' + fmt(available - amount) + '</b>',
