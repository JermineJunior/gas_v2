...
@section('title', 'تقرير تحويلات المستودعات')
...
@section('content')
    <div class="max-w-6xl mx-auto bg-white p-6 mt-10 rounded-lg shadow-md">
        <h2 class="text-xl font-semibold mb-6 text-gray-700">تقرير التحويلات بين المستودعات</h2>

        <form action="{{ route('warehouse_reports.transfers.result') }}" method="POST" class="grid grid-cols-1 md:grid-cols-3 gap-6">
            @csrf

            <div>
                <label class="block text-sm font-medium text-gray-600 mb-1">من مستودع</label>
                <select name="from_warehouse_id" class="w-full select2-from">
            ...
            <div>
                <label class="block text-sm font-medium text-gray-600 mb-1">إلى مستودع</label>
                <select name="to_warehouse_id" class="w-full select2-to">
                    <option value="">الكل</option>
                    @foreach ($warehouses as $warehouse)
                        <option value="{{ $warehouse->id }}">{{ $warehouse->name }}</option>
                    @endforeach
...
@section('scripts')
    ...
    <script>
        ...
        $(document).ready(function() {
            $('.select2-from').select2({ placeholder: "كل المستودعات", allowClear: true, width: '100%' });
            $('.select2-to').select2({ placeholder: "كل المستودعات", allowClear: true, width: '100%' });
        });
    </script>
@endsection
