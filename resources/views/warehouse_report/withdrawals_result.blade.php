...
@section('title', 'نتائج تقرير سحوبات المستودعات')
    ...
    <div class="max-w-6xl mx-auto mt-10 bg-white shadow-lg rounded-xl p-6 print-card">
        ...
        <div class="flex items-center justify-between border-b pb-4 mb-6">
            <div class="flex items-center gap-4">
                ...
                <h2 class="text-2xl font-bold text-gray-700">نتائج تقرير سحوبات المستودعات</h2>
            </div>
            <div class="no-print flex gap-3">
                <a href="{{ route('warehouse_reports.withdrawals') }}"
                ...
                <tbody>
                    @forelse($transactions as $t)
                        <tr class="odd:bg-white even:bg-gray-50 hover:bg-[#F7ECEE] transition">
                            ...
                            <td class="p-3 text-center text-red-700 font-semibold">{{ formatNumber($t->quantity) }}</td>
        ...
        @if ($transactions->count() > 0)
            <div class="mt-6 bg-primary-soft rounded-lg p-4 flex justify-between items-center">
                <span class="text-lg font-bold text-gray-700">إجمالي المسحوبات:</span>
                <span class="text-xl font-bold text-red-700">{{ formatNumber($total) }} لتر</span>
            </div>
        @endif
    </div>
@endsection
