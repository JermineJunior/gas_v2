...
@section('title', 'تقرير سحوبات المستودعات')
...
@section('content')
    <div class="max-w-6xl mx-auto bg-white p-6 mt-10 rounded-lg shadow-md">
        <h2 class="text-xl font-semibold mb-6 text-gray-700">تقرير سحوبات المستودعات</h2>

        <form action="{{ route('warehouse_reports.withdrawals.result') }}" method="POST" class="grid grid-cols-1 md:grid-cols-3 gap-6">
            @csrf
            ...
            <div>
                <label class="block text-sm font-medium text-gray-600 mb-1">اسم السائق</label>
                <input type="text" name="driver_name" placeholder="بحث بالسائق..."
...
@section('scripts')
    ...
    <script>
        ...
        $(document).ready(function() {
            $('.select2').select2({ placeholder: "كل المستودعات", allowClear: true, width: '100%' });
        });
    </script>
@endsection
