...
@section('content')
    <div class="max-w-6xl mx-auto bg-white p-6 mt-10 rounded-lg shadow-md">
        <h2 class="text-xl font-semibold mb-6 text-gray-700">تقرير الموردين</h2>

        <form action="{{ route('reports.supplier.result') }}" method="POST" class="grid grid-cols-1 md:grid-cols-3 gap-6">
            @csrf
            <!-- اختيار المورد -->
            <div>
                <label class="block text-sm font-medium text-gray-600 mb-1">المورد</label>
                <select name="supplier_id" id="supplier_id" class="w-full select2">
            ...
            <div class="md:col-span-3 flex justify-center mt-4">
                <button type="submit"
                    ...
                    @cannot('reports.supplier') disabled @endcannot>
                    بحث
                </button>
            </div>
        </form>
