...
<body class="bg-gray-100 p-6">
    ...
    <div class="max-w-6xl mx-auto bg-white p-6 mt-10 rounded-lg shadow-md">
        <h2 class="text-xl font-semibold mb-6 text-gray-700">تقرير التناكر</h2>

        <form action="{{ route('reports.tuncker.result') }}" method="POST" class="grid grid-cols-1 md:grid-cols-3 gap-6">
            @csrf
            <!-- اختيار المحطة -->
            <div>
                <label class="block text-sm font-medium text-gray-600 mb-1">المحطة</label>
                <select name="station_id" id="station_id" class="w-full select2" {{ auth()->user()->type == 3 ? 'disabled' : '' }}>
                    <option value="">اختر المحطة</option>
                    @foreach ($stations as $station)
                        <option @selected(auth()->user()->type == 3 && $station->id == auth()->user()->stations[0]->pivot->station_id) value="{{ $station->id }}">{{ $station->name }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="md:col-span-3 flex justify-center mt-4">
                <button type="submit"
                    ...
                    @cannot('reports.tuncker') disabled @endcannot>
                    بحث
                </button>
            </div>
        </form>
